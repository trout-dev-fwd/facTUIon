...
- `pick_claim_target(npc_idx)` — best unclaimed wasteland tile adjacent to the NPC's faction's existing territory. Candidates are scored as `distance_to_npc + primary_resource_distance * PRIMARY_RESOURCE_WEIGHT` (weight = 2) so NPCs preferentially extend territory toward their faction's main resource cluster first, then drift to scattered nodes once the main-cluster area is saturated. Skips walled tiles, capital footprints, the NPC's blacklist, and tiles another same-faction NPC is already targeting. Returns `None` if the home capital has less than `CLAIM_SCRAP_COST` scrap.
- `primary_distance_field(faction)` — returns a reference to the precomputed distance-to-nearest-resource vector for the faction's primary resource type: Water → water, Gas → rocky, Scrap → ruins, Cult → `None`.
- `dist_to_water` / `dist_to_rocky` / `dist_to_ruins` — flat `width × height` vectors on `GameState`, populated once in `new()` via the module-level `compute_distance_field` multi-source BFS. Each cell holds the manhattan distance to the nearest tile of the matching resource type. Used by the claim picker; the fields never need updating since resource tiles don't move.
