...
- `step_npc_toward(i, tx, ty)` — single-step movement toward (tx, ty). Calls `bfs_next_step` to find the shortest path around any obstacles (walls, capitals, other NPCs), then takes the first step. Falls back to a random cardinal step via `sim_rng` only if BFS returns `None` (truly unreachable this tick — usually a temporary chokepoint).
- `bfs_next_step(npc_idx, tx, ty)` — BFS search from the NPC to the nearest tile cardinally adjacent to (tx, ty). Uses `is_static_blocked` (walls, capitals, terrain, edges) — NOT `is_blocked_for_npc` — so pathfinding ignores other NPCs and the player. This prevents deadlocks where crowded NPCs all see each other as permanent walls. Collisions are still respected at move time in `step_npc_toward`. Returns `Some((dx, dy))` for the first step, or `None` if unreachable via static terrain.
- `is_static_blocked(x, y)` — walkability check using only static obstacles (edges, non-wasteland terrain, walls, capital footprints). No NPC/player blocking. Only used by BFS.
