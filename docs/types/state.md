...
- `bfs_next_step(npc_idx, tx, ty)` — BFS search from the NPC to the nearest tile cardinally adjacent to (tx, ty). Respects `is_blocked_for_npc` so walls, capitals, the player, and other NPCs all act as obstacles during pathfinding. Returns `Some((dx, dy))` for the first step, or `None` if unreachable. The target tile itself is never entered (BFS only has to *reach* a neighbor — resources and capitals are non-walkable but are what the NPC wants to work *next to*).
