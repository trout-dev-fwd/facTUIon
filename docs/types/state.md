...
- `step_npc_toward(i, target: AstarTarget)` — single-step movement toward an A* goal description. Runs up to 3 passes:
  ...
  3. **Random cardinal step** — last resort for when both A* passes fail or the first step is still blocked by a transient peer collision.
