...
- `try_grow_capital(cap_idx)` (`pub(super)`) — growth check called at any site where a capital's water stockpile could increase: NPC deposit in the `Returning` branch of `update_npcs`, and `actions.rs::sell_resource` when resource == 1 (water). If the capital's water is at or above `WATER_GROWTH_THRESHOLD` **and** a valid spawn tile exists via `find_growth_spawn`, deducts `WATER_GROWTH_COST` and spawns one new NPC assigned to that capital. If no spawn tile is available, growth pauses silently and water is NOT consumed — the next water deposit retries. **Instant** — no timer, so growth feels tied to the action that caused it.
