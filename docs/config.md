...
4. **Economy** — starting stockpiles, `MAX_STOCKPILE` (base cap for a tier-1 city; effective cap = `tier * MAX_STOCKPILE`), `HOARD_BUFFER` (harvest stops this many units below the cap), `MAX_CITY_TIER`, `BASE_UPGRADE_COST` (per-resource cost for a tier upgrade = `current_tier * BASE_UPGRADE_COST`), decay interval, per-resource decay toggles, dehydration interval.
...
6. **NPC behavior** — `MAX_HOARD_BEFORE_USE` (the stockpile threshold at which NPCs stop harvesting a resource type; once every resource is capped the NPC just wanders). Future NPC expenditure behaviors (wall building, trading) will key off this same threshold. `NPC_SCARCITY_WEIGHT` controls how aggressively the target picker prefers scarce resources — the full score is `distance + effective_amount * NPC_SCARCITY_WEIGHT`. Default of 2 means NPCs prefer nearby resources until they actually hit the threshold.
7. **Population growth & upgrades** — `WATER_GROWTH_COST` (water spent per new NPC). Growth triggers when water hits the capital's current `resource_cap()` and population is below `npc_target()`. Once population reaches `npc_target()`, additional water / fuel / scrap accumulate until all three cover `upgrade_cost()`, at which point the city auto-upgrades a tier. All instant — no timer.
