...
7. **Population growth** — `GROWTH_INTERVAL_MS` (how often each capital checks for growth), `WATER_GROWTH_THRESHOLD` (water stockpile required, defaults to `MAX_STOCKPILE`), `WATER_GROWTH_COST` (water spent per new NPC). Capitals that reach the threshold each tick convert the cost into a new NPC assigned to that capital.
